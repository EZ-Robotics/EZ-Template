...
PID::constants_ PID::constants_get() { return constants; }
...
double PID::compute(double current) {
  ...
  if (constants.ki != 0) {
    ...
    if (util::sign(error) != util::sign(error_prev))
      integral = 0;
  }
...
void PID::exit_print(ez::e_exit_output exit_type) {
  std::cout << " ";
  if (is_name)
  ...
  if (exit.velocity_exit_time != 0) {  // Check if this condition is enabled
    if (abs(derivative) <= 0.05) {
      ...
      if (k > exit.velocity_exit_time) {
        timers_reset();
        if (print) exit_print(VELOCITY_EXIT);
